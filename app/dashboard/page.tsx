...
export default function DashboardPage() {
	const { user, userStats, userFavorites, initialized, logout, loadFavorites } =
		useAuth();
	const { giveaways, loading } = useGiveaway();
		...
		const fetchGiveaways = async () => {
			...
			try {
				if (userStats?.claimed_giveaways) {
					const claimed = giveaways.filter((g: Giveaway) =>
						userStats.claimed_giveaways.includes(g.id)
					);
					setClaimedGiveaways(claimed);
				}

				if (userFavorites.length > 0) {
					const favorites = giveaways.filter((g: Giveaway) =>
						userFavorites.includes(g.id)
					);
				...
				<div className="flex justify-between items-start mb-12">
					<div>
						...
						<p className="text-muted-foreground">
							Track your claimed giveaways and manage your account
						</p>
					</div>
					<Button
						onClick={logout}
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-3xl font-bold text-accent">
									${userStats?.total_worth?.toFixed(2) || "0.00"}
								</p>
							</div>
