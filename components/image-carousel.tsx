...
export default function ImageCarousel({ giveaways }: CarouselProps) {
	...
	const validGiveaways = giveaways.filter(
		g => (g.image || g.thumbnail) && g.title
	);
	...
	return (
		...
		>
			{validGiveaways.map((giveaway, index) => (
				<Link
					key={index}
					href={`/giveaway/${currentGiveaway.id}`}
				...
				>
					...
					>
						...
						<div className="absolute bottom-6 left-0 right-0 p-4 sm:p-6 text-white">
							<h2 className="text-lg sm:text-2xl font-bold mb-2 line-clamp-2 text-balance">
								{giveaway.title}
							</h2>
			...
			<button
				onClick={e => {
					e.preventDefault();
					setCurrentIndex(
						prev => (prev - 1 + validGiveaways.length) % validGiveaways.length
					);
					setIsAutoPlay(false);
				}}
