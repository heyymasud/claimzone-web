...
export default function GiveawayCard({ giveaway }: { giveaway: Giveaway }) {
	const { user, isFavorite, addFavorite, removeFavorite, favoriteLoading } =
		useAuth();
					...
					<div className="relative w-full h-48 bg-muted overflow-hidden">
						<Image
							src={
								giveaway.thumbnail ||
								giveaway.image ||
								"/placeholder.svg?height=192&width=400&query=game"
							}
							alt={giveaway.title}
							fill
					...
					<div className="p-4 flex flex-col flex-1">
						...
						<div className="flex gap-2 mb-3 flex-wrap">
							{giveaway.platforms?.split(",")?.map((platform, i) => (
								<span
									key={i}
									className={`text-xs font-semibold text-white px-2 py-1 rounded ${handlePlatformColor(platform.trim())}`}
								>
									{platform.trim()}
								</span>
							))}
							<span
								className={`text-xs font-semibold text-white px-2 py-1 rounded ${typeColor}`}
							>
								{giveaway.type || "Game"}
							</span>
						</div>
						...
						<div className="flex justify-between items-end mt-auto">
							...
							<div
								className={`text-right ${giveaway.end_date != "N/A" ? "" : "invisible"}`}
							>
